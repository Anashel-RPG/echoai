...
The idea was both simple and intriguing: **EchoAI would capture the essence of our dialogues,** the unspoken nuances and the vibrant ideas being woven in the flow of our dialogues. 
...
**Prerequisite Check:** 
- #### Ensure you have install [python version 3.11 ](https://www.python.org/downloads/) or above.
- #### Install [latest version of git](https://git-scm.com/downloads) to be able to clone and download EchoAI
...
- #### Open your terminal found in Applications / Utilities folder
- #### Type "CD" and drag a folder in the terminal window where you want to install EchoAI (Hit Enter)
...
- #### Keep the ' ' character but [] are part of the placeholder you need to replace with your API key
...
The default setting of EchoAI will produce around 12 images and should take 5 to 10 minutes on the first execution as it will setup a download of its python dependencies. Be patient! 

The first run will take about 261 API credits.
- #### On mac, type sudo su in your terminal, hit enter and authenticate your password. Drag the file [start_mac.sh](start_mac.sh) in your terminal window and hit enter
- #### On windows, double-click the file [start_windows.bat](start_windows.bat)

Your EchoAI images will be automatically saved in the local folder **"downloaded_images"**. Each echo image should be 2376 x 1344 pixels landscape format.
> _The log will display this when a new image is added to your downloaded_images_ ![termina_images_done.jpg](readme_img%2Ftermina_images_done.jpg)

The default Echo codex will vary in quality. Customizing it's matrix will help you find a style you like. 

If you want to run another cycle of echo, press the 'up arrow' key in your terminal and hit.
> _Example of echo result with the default codex:_![echo-01.jpg](readme_img%2Fecho-01.jpg)
...
Building your own codex is a creative process, and the EchoAI Matrix is a tool to help achieve this. In the **'config-files/'** folder of your **EchoAI project**, you will find two important file; scene.csv and structure.csv.

**Scene.csv** is a list of scenes that will be used to generate your echo. You can change the default scene or add as many scene as you want.
> **WARNING:** ![scene.jpg](readme_img%2Fscene.jpg) Since this is a csv format, do not include any comma in your scene name, and try to avoid ponctuation. Do not change the first row of your file; it should be name 'Scene Description' as it is case sensitive.

Your second file, **Structure.csv,** represent a matrix of attributes that will be use to create your scene. A matrix is composed of eight categories of attributes. By default, you will have nine values per categories. These will be randomly combined with your scene to explore different visual possibilities. A codex with the default matrix will hold 387 millions possibilities. 
> **WARNING:** ![structure.jpg](readme_img%2Fstructure.jpg) Similar to the Scene.csv, you must not alter the first row and should not include any comma or ponctuation when replacing or adding a new attributes. You cannot change the number of categories or alter their name.
> 
>While you can add as many new attributes, you must keep the same number of attributes per categories.

To fully explore the possibilities of your codex, you should increase your EchoAI multithreading capacity. This will be done in two locations. The first one will be in your [config.py](config.py) file. Find the line MAX_CONCURRENT_JOBS = 1 and change it's value to 3. Note that 5 is the maximum value, but may be less stable. This value manage how many echo image are being generated at the same time. Each echo will still have it's own unique attributes combination.

The second location will be in your [leonardo.json](config-files%2Fleonardo.json) located in your **'config-files/'** folder. Find the line "NUM_IMAGES": 1, and set its value to 2. This value manage how many variation (seed) each echo should have. With the value 2, each unique echo attributes will always return two alternative images results. The maximum value is 4.

Finally, the last is to defined how many echo cycle you wish to run. A cycle is the full production of all your scene. By default, the value is set to 1. You can change this value in your [config.py](config.py) file. Find the line NUM_ITERATIONS = 1 and change the value to the number of cycle you wish to run. There is no max value.

> **Performance**
> - **A Codex of 12 scenes** | scene.csv
> - **With 9 attributes per categories** | structure.csv
> - **Set for 2 cycles** | NUM_ITERATIONS = 2
> - **With 3 concurrents echo** | MAX_CONCURRENT_JOBS = 3
> - **And 2 variations per ech**o | "NUM_IMAGES": 2
>  
> Will generate around 35 images of **2376 × 1344** pixel in about 5 minutes or the equivalent of 1 HD images every 8 seconds. Since they are batched and multi-threaded, they will show i batch update on your downloaded_images folder.

> **Demo output** of the default codex on a 12.9.2.3.2 settings in 5 minutes. (@50% actual size) ![default_codex.jpg](readme_img%2Fdefault_codex.jpg)
---
## Mode 2 Exploration - Refining

![Screenshot of EchoAI](readme_img/cycle38.jpg)  

Your codex may be too limited in scene and attributes to provide a creative exploration, or too vaste to be able to find a style you like. The EchoAI Refining mode is a tool to help you find the right balance.

Put all the image you like in the **ranking_good/** folder and all the image you don't like in the **ranking_bad/** folder. On mac, open your terminal window and drag the **ranking_mac.sh** file. On windows, double-click the **ranking_windows.bat.** 

Depending on the number of image you have, it may take up to 60 seconds to process. The result will show you how often your attributes where present in the image you liked and how often some attributes where involved in the image you did not liked. With small sample, you will see overlap between your good and bad attributes.

> ![attributes.jpg](readme_img%2Fattributes.jpg)

Use this process to replace attributes you do not like or to reduce the amount of attributes (while keeping the same numbers of attributes per categories).

### Locking your style
After enough exploration, you maye fine the exact style you like. It is up to you if you wish to use your codex or if you wish to lock your style before going into conversation mode. If you want to lock a specific style, the steps are very simple.

The echo attributes as well as the style are save in the file properties. Depending on your OS, the data may be truncated when trying to read it via the file properties. Search online for a [exif viewer online website](https://jimpl.com/) and upload your image. If the commments section show "undefined", you may want to try a different viewer.

**In a text editor;**
1. Copy the ImageDescription Echo attributes value
2. Copy the presetStyle: value from the UserComment section.
3. Do not include the trailing ;

**In your [config.py](config.py) file;**
1. Go to **OVERRIDE SETTINGS** section
2. To uncomment a line, remove the # character and it's space at the beginning of the line
2. Uncomment the **OVERRIDE_STYLE**
3. Set it's value to the **presetStyle** value you copied
4. Uncomment the **OVERRIDE_STRING** and set it's value to your echo attributes
5. The echo attribute had your scene description, replace the scene part with <SCENE>
6. Uncomment one **OVERRIDE_MODE**L based on your preference. (Realistic versus artistic)

You are now ready to run your codex trough another cycle.

> Your configuration file with an **OVERRIDE** setting should look like this:
> ![override.jpg](readme_img%2Foverride.jpg)
 
> EXIF used as override style setting
> ![exif.jpg](readme_img%2Fexif.jpg)

> Demo output of an override codex on a 12.9.2.3.2 settings.
...
Now that you have your codex fine tuned (either in open mode or in override mode), it is time to explore the creative universe within your brainstorm meeting. Currently, EchoAI only support text transcript but I hope to ship soon an audio input that can run in realtime during your brainstorming or creative conversation.

EchoAI ship with HP Lovecraft 'Through the Gates of the Silver Key' as a demo transcript. Go to config-files/conversation-capture.txt to load your own transcript using a text editor. You must have a minimum of 20 sentence in your transcript.
...
> 
> You then **must set your main theme** that best describe the universe of your brainstorm. The value should be one sentence with no punctuation and it is advise to start it with 'in the...'
> - config.py / UNIVERSE = "in the dark world of lovecraft"
> 
> Conversation mode does not support multi threading (yet). We recommend using the following setting for your first conversation run:
...
It will first take 5 successive sentences from a random location in your transcript. The following is a **demo output** of an override codex in conversation mode using HP Lovecraft '**Through the Gates of the Silver Key**'
...
### <br> Refining First and Second pass
If you wish to control the behaviour of the first and second pass, open the **trigger.py** file in a text editor and look for **FIRST PASS SETTINGS** and **SECOND PASS SETTINGS** section. The prompt tag represent the instructions given to ChatGPT to transform our original text into a scene that will be render in our Echo pipeline. (Using our attributes or override setting)
...
Running EchoAI in Immersion mode is a way to display in realtime fullscreen version of echo image as they arrive. If you want to broadcast your Echo on a stream, on a monitor or as a webcam feed, you can capture the feed using OBS / Streamlab OBS and set it as a Virtual WebCam (For Slack Huddle, Microsoft Teams Meeting or Google Hangout), set it as a Twitch stream or as a Youtube livestream.
> Immersion mode can only be run using single thread setting with one image per thread. You can chages theses settings with your text editor.
> - config.py / MAX_CONCURRENT_JOBS = 1
> - leonardo.json / "NUM_IMAGES": 1
>
> Once you have set the required setting, you can set the immersion mode in the config.py file.
...
EchoAI was the result of a weekend jam session with ChatGPT driven by a personal need of a greater exploration possibilities in a passive experience. API cost will probably limit access to a larger audience. I hope to add more API bridge outside [Leonardo.ai](https://leonardo.ai/), including [SDNEXT](https://github.com/vladmandic/automatic) to enable your own GPU usage.

If you run an AI image generation services with API capacity and would like to collaborate, please reach me on [Discord](https://discord.gg/arg) user Anashel or on [LinkedIn](https://www.linkedin.com/in/adoyon/). I plan to see if [Mage.space](https://www.mage.space/) and [Civitai.com](https://civitai.com/) API services integration would be possible. 

In the meantime, I invite you to share your own EchoAI codex (scene.csv and structure.csv) or your Override settings with the community, along with the creative result of your work!
...
My AI journey started with the training of the official [RPG model checkpoint](https://civitai.com/models/1116/rpg), now at version 5.0. I also wrote a [41 pages guide](https://civitai.com/models/115223?modelVersionId=124633) on prompting and contributed over [90 RPG ControlNet](https://civitai.com/models/97181?modelVersionId=144429) depth map. You can support me via my [Patreon page](https://www.patreon.com/RPGAI). For any business opportunity, the fastest way to reach me is on my [Discord](https://discord.gg/arg) server / user Anashel.
